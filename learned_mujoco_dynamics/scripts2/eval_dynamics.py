...
from learned_dynamics2.parallel_collector import parse_action_std, sample_smooth_action
from learned_dynamics2.paths import DEFAULT_MODEL_XML, resolve_project_path
from learned_dynamics2.train_utils import build_model, load_checkpoint, set_seed
...
def select_first_prediction_head(pred_delta_norm: torch.Tensor, target_dim: int) -> torch.Tensor:
    if pred_delta_norm.ndim != 2:
        raise ValueError(f"pred_delta_norm must be rank-2, got ndim={pred_delta_norm.ndim}")
    if pred_delta_norm.shape[-1] == target_dim:
        return pred_delta_norm
    if pred_delta_norm.shape[-1] % target_dim != 0:
        raise ValueError(
            f"Prediction dimension {pred_delta_norm.shape[-1]} is not divisible by target_dim={target_dim}"
        )
    return pred_delta_norm.reshape(pred_delta_norm.shape[0], pred_delta_norm.shape[-1] // target_dim, target_dim)[:, 0]
...
def predict_next_state(
    ...
    with torch.no_grad():
        ...
        pred_delta_norm = select_first_prediction_head(pred_delta_norm, target_dim)
