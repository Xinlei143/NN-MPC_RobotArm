...
class DynamicsDataset(Dataset[tuple[torch.Tensor, torch.Tensor]]):
    def __init__(
        ...
        if direct_loss_steps <= 0:
            raise ValueError(f"direct_loss_steps must be positive, got {direct_loss_steps}")
        ...
        self.direct_loss_steps = int(direct_loss_steps)
    ...
    def _build_sequence_indices(self) -> np.ndarray | None:
        if self.episode_ids is None:
            return None
        if self.model_type == "mlp" and self.direct_loss_steps == 1:
            return None
        ...
        valid_runs = (run_ends - run_starts) >= (min_window + self.direct_loss_steps - 1)
        starts_by_run = [
            np.arange(start, end - min_window - self.direct_loss_steps + 2, dtype=np.int64)
        ...
        if not starts_by_run:
            raise ValueError(
                ...
                "each episode must contain enough samples for the requested direct_loss_steps."
            )
    ...
    def _target_window_at(self, index: int) -> torch.Tensor:
        targets = [self._target_at(index + step) for step in range(self.direct_loss_steps)]
    ...
    def __len__(self) -> int:
        if self.model_type == "mlp":
            ...
            return int(self.states.shape[0] - self.direct_loss_steps + 1)
        if self.sequence_indices is not None:
            return len(self.sequence_indices)
        # Backward compatibility for v1 npz files with no episode_ids.
        return int(self.states.shape[0] - self.history_len - self.direct_loss_steps + 2)

    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor]:
        if self.model_type == "mlp":
            current_index = int(self.sequence_indices[index]) if self.sequence_indices is not None else index
            x = torch.cat([self.states[current_index], self.actions[current_index]], dim=0)
            y = self._target_at(current_index) if self.direct_loss_steps == 1 else self._target_window_at(current_index)
            return x, y
        ...
        y = self._target_at(end - 1) if self.direct_loss_steps == 1 else self._target_window_at(end - 1)
        return x, y
...
class RolloutDynamicsDataset(DynamicsDataset):
    def __init__(
        ...
        super().__init__(
            ...
            direct_loss_steps=direct_loss_steps,
        )

    def _build_sequence_indices(self) -> np.ndarray | None:
        horizon_steps = max(self.rollout_steps, self.direct_loss_steps)
        if self.model_type == "mlp" and self.episode_ids is None:
            max_start = int(self.states.shape[0] - horizon_steps)
            if max_start < 0:
                raise ValueError(
                    f"No valid rollout windows for rollout_steps={self.rollout_steps}, "
                    f"direct_loss_steps={self.direct_loss_steps}; "
                    f"dataset has {self.states.shape[0]} samples."
                )
            return np.arange(0, max_start + 1, dtype=np.int64)

        if self.episode_ids is None:
            max_start = int(self.states.shape[0] - self.history_len - horizon_steps + 1)
            if max_start < 0:
                raise ValueError(
                    f"No valid rollout windows for history_len={self.history_len}, "
                    f"rollout_steps={self.rollout_steps}, direct_loss_steps={self.direct_loss_steps}; "
                    f"dataset has {self.states.shape[0]} samples."
                )
            return np.arange(0, max_start + 1, dtype=np.int64)
        ...
        run_ends = np.concatenate((boundaries, [len(episode_ids)]))
        min_window = 1 if self.model_type == "mlp" else self.history_len
        valid_runs = (run_ends - run_starts) >= (min_window + horizon_steps)
        starts_by_run = [
            np.arange(start, end - min_window - horizon_steps + 2, dtype=np.int64)
            for start, end in zip(run_starts[valid_runs], run_ends[valid_runs])
        ]
        if not starts_by_run:
            raise ValueError(
                f"No valid rollout windows for history_len={self.history_len}, "
                f"rollout_steps={self.rollout_steps}, direct_loss_steps={self.direct_loss_steps}; "
                "each episode must contain enough samples."
            )
        return np.concatenate(starts_by_run)
