...
def fig2(output: Path, data_root: Path, arrays: dict[str, dict[str, np.ndarray]], seed: int, paths: dict[str, Path]) -> None:
    ...
    center, basis = pca_plane(desired)
    project = lambda values: (values - center) @ basis.T * 1000.0
    fig, axes = plt.subplots(2, 1, figsize=(SINGLE_COLUMN_IN, 3.05))
    ax = axes[0]
    for name in ("Desired", "NaiveDelayed", "ThreadedASAP", "Projected Direct IK", "Preview IK"):
        values = desired if name == "Desired" else arrays[name]["actual_ee_positions"]
        xy = project(values)
        ax.plot(xy[:, 0], xy[:, 1], label=name, **method_style(name, markevery=90 if name != "Desired" else None))
    ax.set_xlabel("Task-plane coordinate 1 [mm]"); ax.set_ylabel("Task-plane coordinate 2 [mm]"); ax.set_aspect("equal", adjustable="box")
    finish_axis(ax); panel_label(ax, "a")
    ax = axes[1]
    ...
    for name in ordered:
        ...
        ax.plot(time, values["ee_position_errors"] * 1000, label=name, **method_style(name, markevery=100))
    ax.set_xlabel("Time [s]"); ax.set_ylabel("TCP position error [mm]"); finish_axis(ax); panel_label(ax, "b")
    rmses = ", ".join(f"{name}: {np.sqrt(np.mean(arrays[name]['ee_position_errors']**2))*1000:.1f}" for name in ("NaiveDelayed", "FullVirtual", "ThreadedASAP"))
    ax.text(0.02, 0.97, f"RMSE [mm]\n{rmses}", transform=ax.transAxes, va="top", fontsize=5.8,
            bbox={"facecolor": "white", "edgecolor": "#B0B0B0", "alpha": .9, "pad": 2})
    handles, labels = axes[1].get_legend_handles_labels()
    fig.legend(handles, labels, loc="lower center", bbox_to_anchor=(.51, .005), ncol=2, fontsize=5.9, handlelength=1.9, columnspacing=.9)
    fig.subplots_adjust(hspace=.36, left=.18, right=.985, bottom=.23, top=.98)
    save_figure(fig, output / "fig2_representative_tracking")
    source = [path / "rollout.npz" for path in paths.values()]
    metadata = source_metadata(source, selection=f"Pre-registered nominal circle seed {seed}, closest ThreadedASAP RMSE to five-seed median.", statistic_unit="Representative trajectory; plotted 10-ms samples are descriptive, not inferential.", transform="One PCA/SVD plane fitted once to desired TCP positions; all methods use the same two basis vectors; metres converted to mm.")
    metadata.update({"methods": ["Desired", *ordered], "seed": seed, "raw_curve_smoothing": "none"})
    tidy = []
    for name in ("Desired", *ordered):
        ...
        for step, (x, y) in enumerate(project(positions)):
            tidy.append({"panel": "task_plane", "method": name, "step": step, "coordinate_1_mm": x, "coordinate_2_mm": y})
        if name != "Desired":
...
def fig5(output: Path, data_root: Path, paper_root: Path) -> None:
    ...
    ax.axvline(0,color="#555555",ls="--",lw=.8); ax.axvspan(-32,0,color="#EAF3EA",alpha=.38,zorder=0); ax.text(-30.5,4.55,"ThreadedASAP better",fontsize=6.1,color="#4D4D4D")
    ax.set_yticks(ypos); ax.set_yticklabels([x[1] for x in order]); ax.set_xlabel("Δ TCP RMSE [mm] (ThreadedASAP − IK)"); ax.set_xlim(-32,6); finish_axis(ax,grid=False); ax.grid(axis="x",color="#B9B9B9",alpha=.18,lw=.55); ax.legend(loc="lower left",fontsize=6.5)
    ...
    write_bundle(data_root/"fig5",tidy=tidy,wide=pd.DataFrame(tidy).pivot(index="condition",columns="baseline",values="delta_tcp_rmse_mm").reset_index().to_dict("records"),metadata=metadata,readme="Fig. 5 forest-plot data. Negative values favor ThreadedASAP; observation noise versus Preview IK crosses zero.")
