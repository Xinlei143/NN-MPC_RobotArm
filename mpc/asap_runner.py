...
def run(args: Any, api: dict[str, Any]) -> dict[str, Any]:
        ...
        for step in range(execution_steps):
            ...
            if active is not None:
                ...
                if index is not None:
                    ...
                    uncertainty_gate = bool(active.uncertainty_gate)
                    uncertainty_score = float(active.uncertainty_score)
                    uncertainty_max_score = float(active.uncertainty_max_score)
                    uncertainty_evaluation_time = float(active.uncertainty_evaluation_time_s)
                    event = "packet_active"
            feedback = np.zeros(args.n_joints, dtype=np.float32) if age < 0 or uncertainty_gate else feedback_correction(predicted, state, args.feedback_kq, args.feedback_kdq, feedback_max)
    ...
    arrays["uncertainty_threshold"] = np.asarray(args.uncertainty_threshold, dtype=np.float32)
    arrays["uncertainty_ensemble_size"] = np.asarray(5 if args.uncertainty_mode == "ensemble_gate" else 0, dtype=np.int64)
    if task_reference is not None:
