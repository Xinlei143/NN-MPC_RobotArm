...
def run(args: Any, api: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        reference, dq_reference, ddq_reference, execution_steps, task_reference = api["_reference_for_run"](args=args, state=state, env=env, control_dt=env.control_dt)
        ...
        for step in range(execution_steps):
            ...
            replan_deadline_miss = int(replanned and planning_time >= env.control_dt * args.anticipation_delay_steps - args.planner_guard_ms / 1000.0)
            ...
            tracking = float(np.linalg.norm(state[:args.n_joints] - reference[step + 1]))
            rec["actual_states"].append(current_state); rec["next_states"].append(state.copy()); rec["q_des"].append(reference[step].copy()); rec["dq_des"].append(dq_reference[step].copy())
