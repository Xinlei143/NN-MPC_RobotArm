...
def run(args: Any, api: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        execution_steps = min(
            execution_steps,
            reference.shape[0] - args.horizon - args.anticipation_delay_steps - int(args.mpc_preview_nominal_steps) - 1,
        )
        ...
        for step in range(execution_steps):
            ...
            nominal = nominal_command(reference, step, int(args.mpc_preview_nominal_steps))
            # A packet's absolute q_ref was formed at its activation anchor.
            # Its residual is the transferable quantity: reconstruct a fresh
            # reference from the current (possibly previewed) IK nominal.
            feedback_reference = nominal + plan_residual
            ...
            for source, target in (("actuator_tau", "tau_actuator"), ("gravity_tau", "tau_gravity"), ("total_tau", "tau_total"), ("true_gravity_tau", "tau_gravity_true"), ("gravity_mismatch_tau", "tau_gravity_mismatch")):
                value = torque.get(source)
                if value is None:
                    value = np.zeros(args.n_joints, dtype=np.float32)
                rec[target].append(np.asarray(value, dtype=np.float32))
