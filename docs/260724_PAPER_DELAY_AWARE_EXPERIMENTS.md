...
论文配置显式使用 GRU、history 16、H20、128 candidates、2 CEM iterations、每5个逻辑 tick 重规划一次，以及 `planner_projection=on`、`backend=compiled`、`strategy=two_stage`。根 CLI 也以 GRU 为默认值，但正式 workflow 仍必须显式冻结这些字段。正式 manifest 必须在 clean worktree 上生成，并记录 commit、checkpoint/reference hash、Python、PyTorch、CUDA、MuJoCo、GPU 和 driver。
...
```text
outputs/paper_delay_aware_two_stage_v1/manifests/paper.json
```

文件不可覆盖。配置或 commit 改变时必须使用新的输出根目录，例如 `outputs/paper_delay_aware_two_stage_v2/`。
...
旧 projection-off 结果仅用于复现历史行为；正式配置使用 compiled two-stage projection，且 100 Hz 执行层的 shared physical projection 始终开启。
...
`planner_execution_qref_error` 记录。two-stage 的第一阶段以低成本近似筛选完整 CEM
population，第二阶段对 final elites、mean、best 和 baseline 做精确 compiled projection
与重新评估；它不改变执行层始终存在的 physical safety filter。

下表的 `off` 结果是历史诊断，不是当前论文配置。它曾用于同一旧 GRU、circle、D7
