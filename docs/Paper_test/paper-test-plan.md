...
> 详细操作与指标定义见 [Delay-Aware MPC 论文实验操作手册](../experiments/paper-delay-aware-experiments.md)。
...
旧 `outputs/paper_delay_aware` 是历史 projection-off suite，**不是当前默认测试配置**。它曾使用：
...
- H20、128 samples、2 CEM iterations、history length 16；
- `planner_projection=on`、`planner_projection_backend=compiled`、`planner_projection_strategy=two_stage`。
...
| checkpoint / normalizer | `dynamics_modeling/outputs/checkpoints/gru_20260717_182930/best_model.pt` / `normalizer.pt` |
...
选择规则：只在独立 calibration reference 上，从姿态 RMSE 不超过最优值 10% 的候选中选择最小 TCP RMSE；完全相同时选择更小 preview。
