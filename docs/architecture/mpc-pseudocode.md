...
barrier 对时域采用 `weighted mean + max` 聚合；预测硬越界候选直接无效。默认 terminal weight 是零，避免短 horizon 的末端 learned-model 误差被过度放大。完整公式、默认权重和尺度见 [Cost function](cost-function.md)。
